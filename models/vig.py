...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.stem = Stem(in_dim=3, out_dim=channels, act=act) 
        self.cifarstem = CIFARStem(in_dim=3, out_dim=channels, act=act)
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(4096, channels, 4, 4))

        # instead of manually finding the batch size, generate a functional that waits until the input batch size is known and replace the batch dimension with it
    ...
    def forward(self, inputs):
        ...
        # print("self.cifarstemp input shape", self.cifarstem(inputs).shape)
        # print("self.pos_embed", self.pos_embed.shape)
        try:
            x = self.cifarstem(inputs) + self.pos_embed
        except:
            print("self.cifarstem input shape", self.cifarstem(inputs).shape)
            print("self.pos_embed", self.pos_embed.shape)
